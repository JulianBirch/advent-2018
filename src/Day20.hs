...
import Utility((<$$>))
import Control.Lens (over, Ixed(..), Index, IxValue, makeLenses, preview, use, view, Iso', Traversal', review)
...
-- Not a particularly natural way of expressing the Path. 
-- Don't really know how to improve on it, though
data Path d a = 
    Sequence d a
...
class (Ixed g) => Grid g where
    ...
    storageVector :: (Index g) -> V.Vector2D Int

{-# INLINE gridIx #-}
gridIx :: forall g . (Grid g) => (Index g) -> Traversal' g (IxValue g)
gridIx v = gridIso . (ix x) . (ix y) where
    (V.Vector2D x y) = storageVector @g v 

newtype SquareGrid a = SquareGrid (IM.IntMap (IM.IntMap a)) deriving (Show)

deriveShow1 ''SquareGrid

type instance Index (SquareGrid a) = G.Square Int
type instance IxValue (SquareGrid a) = a

-- More I think about it, the less I think "Vector" is going to last in its fundeps incarnation
instance Grid (SquareGrid a) where
    {-# INLINE gridIso #-}
    gridIso = U.coerceIso
    {-# INLINE storageVector #-}
    storageVector = coerce

instance Ixed (SquareGrid a) where
    ix = gridIx

gridMappend :: forall g . (Semigroup (IxValue g), Grid g) => g -> g -> g
gridMappend = (review gridIso .) . on combine (view gridIso) where
    combine = IM.unionWith (IM.unionWith (<>))

gridConcat :: forall f g . (Semigroup (IxValue g), Grid g, Foldable f, Functor f) => f g -> g
gridConcat = review gridIso . combine . fmap (view gridIso)
    where combine = IM.unionsWith (IM.unionWith (<>))

instance (Semigroup a) => Semigroup (SquareGrid a) where
    (<>) = gridMappend 
    sconcat = gridConcat

instance (Semigroup a) => Monoid (SquareGrid a) where
    mappend = gridMappend
    mconcat = gridConcat
    mempty = SquareGrid $ mempty

data DoorStructure g = DoorStructure {
    _finalPositions :: NE.NonEmpty (Index g),
    _doors :: g
}

deriving instance (Show (Index g), Show g) => Show (DoorStructure g) 

makeLenses 'DoorStructure

translate :: forall g . (Grid g) => Index g -> g -> g
translate v = over gridIso $ IM.mapKeysMonotonic (+x) . fmap (IM.mapKeysMonotonic (+y)) where
    (V.Vector2D x y) = storageVector @g v
   
translateDS :: forall g . (Grid g, Semigroup (Index g)) => Index g -> DoorStructure g -> DoorStructure g
translateDS v = liftA2 DoorStructure fp ds where
    fp = (v <>) <$$> (view finalPositions)
    ds = (translate v) <$> (view doors)

parallelCombine :: (Grid g, Semigroup (IxValue g), Eq (Index g), Monoid g) => NE.NonEmpty (DoorStructure g) -> DoorStructure g
parallelCombine dss = DoorStructure fp ds where
    fp = NE.nub $ (view finalPositions) =<< dss
    ds = foldMap (view doors) dss

serialCombine :: (Grid g, Semigroup (IxValue g), Monoid g, Semigroup (Index g), Eq (Index g)) => DoorStructure g -> DoorStructure g -> DoorStructure g
...
toIntMapPresorted :: (Foldable t) => (v -> Int) -> t v -> IM.IntMap (NonEmpty v)
toIntMapPresorted getKey = foldMap (IM.singleton <$> getKey . NE.head <*> id) . NE.groupWith getKey

toGrid :: forall g t v . (Semigroup (IxValue g), Foldable t, Grid g) => (v -> Index g) -> (v -> IxValue g) -> t v -> g
toGrid toI toS l = review gridIso $ sconcat . fmap snd <$$> toIntMapPresorted (V._y . fst) <$> toIntMapPresorted (V._x . fst) input where
    input = sortOn fst $ (liftA2 (,) (storageVector @g . toI) toS ) <$> toList l
...
gridNeighbours :: forall g p d . (Grid g, V.Vector p d, Index g ~ p, IxValue g ~ S.Set d) => g -> p -> [p]
gridNeighbours g v = traverse V.move directions v where
    directions :: [d]
    directions = foldMap S.toList $ preview (ix (coerce v)) g
          
toDoorStructure :: forall g d . (V.Direction d, Monoid (Index g), V.Vector (Index g) d, Grid g, IxValue g ~ S.Set d) => [d] -> DoorStructure g
toDoorStructure path = f $ process path mempty where
    process = runState . fmap join . traverse (state . processMove)
    f :: ([(Index g, d)], Index g) -> DoorStructure g
    f (pairs, p) = DoorStructure (p :| []) (toG pairs)
    toG :: [(Index g, d)] -> g
    toG = toGrid fst (S.singleton . snd)

path2DoorStructure :: (Grid g, Monoid g, V.Vector (Index g) d, IxValue g ~ (S.Set d), ds ~ DoorStructure g) => (Path [d] ds) -> ds
...
day20 = floodFillDistances <$> (toNeighbours <$> day20FixPath) <*> (pure mempty) where
    toNeighbours = gridNeighbours @(SquareGrid (S.Set G.SquareDirection)) . _doors . cata path2DoorStructure
